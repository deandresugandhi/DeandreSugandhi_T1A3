    ...
    def vertical_victory(self):
        for i_row in range(3): 
            for i_column in range(7):
                if all(self._board._array[i_row:i_row + 4, i_column]):
                    return self._piece1._player_name
                elif all(slot == 2 for slot in self._board._array[i_row:i_row + 4, i_column]):
                    return self._piece2._player_name
        return None
    ...
    def diagonal_victory(self):
        for i_row in range(3):
            for i_column in range(4):
                if all(self._board._array[i + i_row, i + i_column] for i in range(4)) or all(self._board._array[i + i_row, (3 + i_column) - i] for i in range(4)):
                    return self._piece1._player_name
                elif all(slot == 2 for slot in (self._board._array[i + i_row, i + i_column] for i in range(4))) or all(slot > 1 for slot in (self._board._array[i + i_row, (3 + i_column) - i] for i in range(4))):
                    return self._piece2._player_name
        return None
